...
const Home = () => {
  ...
  const fetchFavoriteRoutes = useCallback(async () => {
    if (!userId) return;
    try {
      const res = await fetch(`http://localhost:5000|https://cse471-project-backend-51jt.onrender.com/api/users/${userId}/favorite-routes`);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      if (!userId) return;
      const res = await fetch(`http://localhost:5000|https://cse471-project-backend-51jt.onrender.com/api/users/${userId}`);
      const data = await res.json();
      if (res.ok) {
        setUser(data.user);
    ...
    const fetchNotifications = async () => {
      if (!userEmail) return;
      try {
        const res = await fetch(`http://localhost:5000|https://cse471-project-backend-51jt.onrender.com/api/notifications/user/${userEmail}`);
        if (res.ok) {
          const data = await res.json();
