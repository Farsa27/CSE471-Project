...
export default function UploadAd() {
  ...
  const fetchAds = async () => {
    try {
      const res = await axios.get("https://cse471-project-production.up.railway.app/api/ads");
      setAds(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        // Update existing ad
        await axios.put(`https://cse471-project-production.up.railway.app/api/ads/${editingId}`, data, {
        ...
        await axios.post("https://cse471-project-production.up.railway.app/api/ads", data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://cse471-project-production.up.railway.app/api/ads/${id}`);
  ...
  const handleToggleStatus = async (id) => {
    try {
      await axios.patch(`https://cse471-project-production.up.railway.app/api/ads/${id}/toggle`);
      fetchAds();
    } catch (err) {
  ...
  const handleEdit = (ad) => {
    ...
    setImagePreview(`https://cse471-project-production.up.railway.app${ad.imageUrl}`);
  };
            ...
            {ads.map((ad) => (
              <div key={ad._id} className={`ad-card ${!ad.isActive ? "inactive" : ""}`}>
                <div className="ad-image">
                  <img src={`https://cse471-project-production.up.railway.app${ad.imageUrl}`} alt={ad.title} />
