...
export default function AdSlider() {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const res = await axios.get("https://cse471-project-production.up.railway.app/api/ads/active");
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full bg-white shadow-inner p-4 flex items-center justify-center gap-4">
      ...
      >
        <img
          src={`https://cse471-project-production.up.railway.app${ads[currentIndex].imageUrl}`}
